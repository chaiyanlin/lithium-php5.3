...
class StringTest extends \lithium\test\Unit {
	...
	public function testInsert() {
		...
		$result = LIString::insert($string, array('sum' => '4', 'adjective' => 'yummy'));
		$this->assertEqual($expected, $result);

		$string = '2 + 2 = %sum. Lithium is %adjective.';
		$result = LIString::insert($string, array('sum' => '4', 'adjective' => 'yummy'), array(
			'before' => '%', 'after' => ''
		));
		$this->assertEqual($expected, $result);
		...
		$string = '{:web} {:web_site}';
		$expected = 'www http';
		$result = LIString::insert($string, array('web' => 'www', 'web_site' => 'http'));
		$this->assertEqual($expected, $result);
		...
		$string = ':a :b \:a :a';
		$expected = '1 2 :a 1';
		$result = LIString::insert($string, array('a' => 1, 'b' => 2), array(
			'before' => ':', 'after' => '', 'escape' => '\\'
		));
		$this->assertEqual($expected, $result);
		...
		$string = '{:a} {:b} {:c}';
		$expected = '2 3';
		$result = LIString::insert($string, array('b' => 2, 'c' => 3), array('clean' => true));
		$this->assertEqual($expected, $result);

		$string = '{:a} {:b} {:c}';
		$expected = '1 3';
		$result = LIString::insert($string, array('a' => 1, 'c' => 3), array('clean' => true));
		$this->assertEqual($expected, $result);
		...
		$string = ':a, :b and :c';
		$expected = '2 and 3';
		$result = LIString::insert($string, array('b' => 2, 'c' => 3), array(
		...
		$string = '"{:a}, {:b} and {:c}"';
		$expected = '"1, 2"';
		$result = LIString::insert($string, array('a' => 1, 'b' => 2), array('clean' => true));
		$this->assertEqual($expected, $result);
		...
		$result = LIString::insert($string, array('src' => 'foo'), array('clean' => 'html'));
		$this->assertEqual($expected, $result);

		$string = '<img src=":src" class=":no :extra"/>';
		$expected = '<img src="foo"/>';
		$result = LIString::insert($string, array('src' => 'foo'), array(
		...
		$result = LIString::insert($string, array('src' => 'foo', 'extra' => 'bar'), array(
			'clean' => 'html'
		));
		$this->assertEqual($expected, $result);

		$result = LIString::insert("this is a ? string", array("test"));
		$expected = "this is a test string";
		$this->assertEqual($expected, $result);

		$result = LIString::insert("this is a ? string with a ? ? ?", array(
		...
		$result = LIString::insert('{:me} lithium. {:subject} {:verb} fantastic.', array(
			'me' => 'I :verb', 'subject' => 'lithium', 'verb' => 'is'
		));
		$expected = "I :verb lithium. lithium is fantastic.";
		$this->assertEqual($expected, $result);

		$result = LIString::insert(':I.am: :not.yet: passing.', array('I.am' => 'We are'), array(
		...
		$result = LIString::insert('?-pended result', array('Pre'));
		$expected = "Pre-pended result";
		$this->assertEqual($expected, $result);
	}

	/**
	 * Tests that text replacements with `LIString::insert()` using key/value pairs are not
	...
	public function testInsertWithUnusedNumericKey() {
		$result = LIString::insert("Hey, what are you tryin' to {:action} on us?", array(
			'action' => 'push', '!'
		));
	...
	/**
	 * Tests casting/inserting of custom objects with `LIString::insert()`.
	 */
	public function testInsertWithObject() {
		$foo = new MockStringObject();
		$result = LIString::insert('This is a {:foo}', compact('foo'));
	...
	public function testInsertWithEmptyArray() {
		$result = LIString::insert("Hey, what are you tryin' to {:action} on us?",
			array('action' => array())
		);
	...
	public function testCleanInsert() {
		$result = LIString::clean(':incomplete', array(
			'clean' => true, 'before' => ':', 'after' => ''
		));
		...
		$result = LIString::clean(':in.complete', array(
			'clean' => true, 'before' => ':', 'after' => ''
		));
		$this->assertEqual('', $result);

		$result = LIString::clean(':in.complete and', array(
			'clean' => true, 'before' => ':', 'after' => ''
		));
		$this->assertEqual('', $result);

		$result = LIString::clean(':in.complete or stuff', array(
			'clean' => true, 'before' => ':', 'after' => ''
		));
		$this->assertEqual('stuff', $result);

		$result = LIString::clean(
		...
		$string = ':a 2 3';
		$result = LIString::clean($string, array('clean' => true, 'before' => ':', 'after' => ''));
		$this->assertEqual('2 3', $result);
	}

	public function testTokenize() {
		$result = LIString::tokenize('A,(short,boring test)');
		$expected = array('A', '(short,boring test)');
		$this->assertEqual($expected, $result);

		$result = LIString::tokenize('A,(short,more interesting( test)');
		$expected = array('A', '(short,more interesting( test)');
		$this->assertEqual($expected, $result);

		$result = LIString::tokenize('A,(short,very interesting( test))');
		$expected = array('A', '(short,very interesting( test))');
		$this->assertEqual($expected, $result);

		$result = LIString::tokenize('"single tag"', array(
			'separator' => ' ', 'leftBound' => '"', 'rightBound' => '"'
		));
		$expected = array('"single tag"');
		$this->assertEqual($expected, $result);

		$result = LIString::tokenize('tagA "single tag" tagB', array(
			'separator' => ' ', 'leftBound' => '"', 'rightBound' => '"'
		));
		$expected = array('tagA', '"single tag"', 'tagB');
		$this->assertEqual($expected, $result);

		$result = LIString::tokenize(array());
		$expected = array();
		$this->assertEqual($expected, $result);

		$result = LIString::tokenize(null);
		$this->assertNull($result);
	}

	/**
	 * Tests the `LIString::extract()` regex helper method.
	 */
	public function testStringExtraction() {
		$result = LIString::extract('/string/', 'whole string');
		$this->assertEqual('string', $result);

		$this->assertFalse(LIString::extract('/not/', 'whole string'));
		$this->assertEqual('part', LIString::extract('/\w+\s*(\w+)/', 'second part', 1));
		$this->assertNull(LIString::extract('/\w+\s*(\w+)/', 'second part', 2));
	}

	public function testStringInsertWithQuestionMark() {
		$result = LIString::insert('some string with a ?', array());
		$this->assertEqual('some string with a ?', $result);

		$result = LIString::insert('some {:param}string with a ?', array('param' => null));
		$this->assertEqual('some string with a ?', $result);
	}
	...
	public function testRandom64Generator() {
		...
		for ($i = 0; $i < $count; $i++) {
			$result = LIString::random(8, array('encode' => LIString::ENCODE_BASE_64));
			$this->assertPattern($pattern, $result);
			$this->assertFalse(in_array($result, $check));
			$check[] = $result;
		}
	...
	/**
	 * Tests hash generation using `LIString::hash()`.
	 */
	public function testHash() {
		...
		$expected = sha1($value);
		$result = LIString::hash($value, array('type' => 'sha1'));
		$this->assertEqual($expected, $result);

		$result = LIString::hash($value, array('type' => 'sha1') + compact('salt'));
		$this->assertEqual(sha1($salt . $value), $result);
		$this->assertEqual(md5($value), LIString::hash($value, array('type' => 'md5')));

		$result = LIString::hash($value, array('type' => 'md5') + compact('salt'));
		...
		try {
			$result = LIString::hash($value, array('type' => 'sha256'));
			$this->assertEqual($sha256($value), $result);

			$result = LIString::hash($value, array('type' => 'sha256') + compact('salt'));
		...
		$result = LIString::hash($string, compact('key'));
		$this->assertEqual($expected, $result);
	...
	public function testCompare() {
		...
		$this->assertTrue(LIString::compare('Foo', 'Foo'));
		$this->assertFalse(LIString::compare('Foo', 'foo'));
		$this->assertFalse(LIString::compare('Foo', 'Bar'));

		$this->assertTrue(LIString::compare('', ''));
		$this->assertFalse(LIString::compare('', '0'));
		$this->assertFalse(LIString::compare('0', ''));

		$this->assertException('/to be (a )?string/', function() {
			LIString::compare(null, null);
		});
		$this->assertException('/to be (a )?string/', function() {
			LIString::compare(null, '');
		});
		$this->assertException('/to be (a )?string/', function() {
			LIString::compare('', null);
		});

		$this->assertTrue(LIString::compare('1', '1'));
		$this->assertException('/to be (a )?string/', function() {
			LIString::compare('1', 1);
		});
		$this->assertException('/to be (a )?string/', function() {
			LIString::compare(1, '1');
		});
	...
	/**
	 * Verifies that `LIString::insert()` doesn't completely ignore empty values.
	 */
	public function testInsertingEmptyValues() {
		$this->assertEqual('value="0"', LIString::insert('value="{:value}"', array('value' => 0)));
	}
}
