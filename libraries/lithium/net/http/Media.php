...
class Media extends \lithium\core\StaticObject {
	 ...
	 *          will use the default path.  The values should be `LIString::insert()`-style
	 *          paths or an array of `LIString::insert()`-style paths.  If it is an array,
	 ...
	 *        - `'paths'`: An array of key/value pairs where the keys are `LIString::insert()`
	 ...
	 *          `LIString::insert()` formatting. See `Media::$_assets` for more.
	...
	public static function path($path, $type, array $options = array()) {
		...
		if ($path[0] === '/') {
			...
			$file = LIString::insert($template, $insert);
		}
