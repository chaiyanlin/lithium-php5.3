...
abstract class Database extends \lithium\data\Source {
	 ...
	 *
	 * @see lithium\util\LIString::insert()
	 ...
	 *              escaped, quoted, and inserted into `$query` using `LIString::insert()`.
	...
	public function create($query, array $options = array()) {
		return $this->_filter(__METHOD__, compact('query', 'options'), function($self, $params) {
			...
			if (is_object($query)) {
				...
				$query = LIString::insert($query, $self->value($params['options']));
			}
	...
	public function read($query, array $options = array()) {
		...
		return $this->_filter(__METHOD__, compact('query', 'options'), function($self, $params) {
			...
			if (is_string($query)) {
				$sql = LIString::insert($query, $self->value($args));
			} else {
	...
	public function delete($query, array $options = array()) {
		return $this->_filter(__METHOD__, compact('query', 'options'), function($self, $params) {
			...
			if ($isObject) {
				...
				$sql = LIString::insert($query, $self->value($params['options']));
			}
			$result = (boolean) $self->invokeMethod('_execute', array($sql));
	...
	public function renderCommand($type, $data = null, $context = null) {
		...
		$template = LIString::insert($template, $placeholders, array('clean' => true));
		return trim(LIString::insert($template, $data, array('before' => '{')));
	}
	...
	protected function _operator($key, $value, array $schema = array(), array $options = array()) {
		...
		switch (true) {
			case (isset($config['format'])):
				return $key . ' ' . LIString::insert($config['format'], $values);
	...
	protected function _constraint($name, $value, $schema = null) {
		...
		return trim(LIString::insert($template, $data, array('clean' => array('method' => 'text'))));
	}
