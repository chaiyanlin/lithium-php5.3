...
class FormSignature {
	...
	protected static function _signature($data) {
		...
		$key = $LIString::hash(date('YMD'), array('key' => $key, 'raw' => true));
		$key = $LIString::hash('li3,1_form', array('key' => $key, 'raw' => true));

		return $LIString::hash($data, array('key' => $key));
	}
