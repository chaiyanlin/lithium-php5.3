...
class RequestToken {
	 ...
	 *
	 * @see lithium\util\LIString::hash()
	 ...
	 *                custom salt value to be used by `LIString::hash()`.
	 *              - `'type'` _string_: The hashing algorithm used by `LIString::hash()` when
	...
	public static function get(array $options = array()) {
		...
		if ($options['regenerate'] || !($token = $session::read($options['sessionKey']))) {
			$token = LIString::hash(uniqid(microtime(true)), $options);
