...
class LIString {
	...
	/**
	 * Option flag used in `LIString::random()`.
	 */
	 ...
	 *
	 * ```
	 * $bits = LIString::random(8); // 64 bits
	 ...
	 *              - `'encode'` _integer_: If specified, and set to `LIString::ENCODE_BASE_64`, the
	...
	/**
	 * Initializes `LIString::$_source` using the best available random number generator.
	 *
	 ...
	 *
	 * @see lithium\util\LIString::$_source
	 ...
	 *        - `'clean'`: A boolean or array with instructions for `LIString::clean()`.
	...
	/**
	 * Cleans up a `LIString::insert()` formatted string with given `$options` depending
	 * on the `'clean'` option. The goal of this function is to replace all whitespace
	 * and unneeded mark-up around place-holders that did not get replaced by `LIString::insert()`.
	 *
